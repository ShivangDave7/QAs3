...
async function seedUser () {
  try {
    let user = await User.findOne({
      'username':'Bennie.Hodkiewicz',
      'isDeleted':false,
      'isActive':true
    });
    if (!user || !user.isPasswordMatch('6hFC8eHgZasPrYF') ) {
      let user = new User({
        'password':'6hFC8eHgZasPrYF',
        'username':'Bennie.Hodkiewicz',
    ...
    let admin = await User.findOne({
      'username':'Moriah.Kertzmann',
      'isDeleted':false,
      'isActive':true
    });
    if (!admin || !admin.isPasswordMatch('Jaeq2uvzl9N7p3w') ) {
      let admin = new User({
        'password':'Jaeq2uvzl9N7p3w',
        'username':'Moriah.Kertzmann',
...
async function seedUserRole (){
  try {
    ...
    if (user && user.isPasswordMatch('6hFC8eHgZasPrYF') && userRole){
    ...
    if (admin && admin.isPasswordMatch('Jaeq2uvzl9N7p3w') && adminRole){
      let count = await UserRole.countDocuments({
        userId: admin.id,
        roleId: adminRole.id
      });
